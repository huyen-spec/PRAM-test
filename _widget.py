...
class ExampleQWidget(QWidget):
    ...
    def save_fl(self):   
        ...
        if gt_layer.name.endswith("_lab") or gt_layer.name.endswith("_GT"):  
            if sv_layer.name  == gt_layer.name.replace("_lab", ""):
                ...
                # print("not match")
                # dlg = QDialog()
                # dlg.setWindowTitle("The annotation and image folder names not matched")
                # dlg.exec()
        ...
        # label_stk = self.viewer.layers[f"{sv_layer.name}_lab"].data
        ...
        stk_file = save_folder2 + "annotation.json"
    ...
    def _save_annotate(self):
        sv_layer = self.viewer.layers[self.c_controller.combox.value.name]
        # sv_layer = self.viewer.layers[self.sv_layer_combo.currentText()]

        # print(type(sv_layer.source.path))
        inp_path = sv_layer.source.path
        save_folder = inp_path.replace(inp_path.split("/")[-1], "") + "GT/"

        # create a ground truth folder
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)

        # way1
        # outFile = save_folder + inp_path.split("/")[-1].replace('.tiff', '.json')
        ...
        outFile, _ = QFileDialog.getSaveFileName(
            ...
            + inp_path.split("/")[-1].replace(".tiff", ".json"),
            filter="JSON files (*.json)",
        )

        data = {"img_name": f"{sv_layer.name}_lab"}

        # # if CONFIG.layer_name_label in self.v.layers:
        data["labels"] = self.viewer.layers[f"{sv_layer.name}"].data.tolist()
    ...
    #     # Opening JSON file
    #     json_path = inp_path.replace(inp_path.split("/")[-1], '') + "1_well_labels"

    #     label_stk = []

    #     for i, fl in enumerate(Path(json_path).glob("*.json")):

    #         f = open(fl)
    #         data = json.load(f)

    #         label_stk += data["labels"]

    #     label_stk = np.array(label_stk).reshape()
...
def get_better_contrast(
    ...
    with progress(total=0):
        ...
        # threshold.value(img_layer.data.compute())
        binarised_im = img_layer.data  # > threshold_val
        # seg_labels = da.from_array(label(binarised_im))
        seg_labels = binarised_im

        # seg_labels = img_layer
        

    seg_layer = (seg_labels, {"name": f"{img_layer.name}_norm"}, "image")
...
# https://programtalk.com/python-more-examples/magicgui.widgets.ComboBox/
...
# git remote add origin https://github.com/huyen-spec/PRAM-visualize.git
# git branch -M main
# git remote set-url origin https://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@github.com/huyen-spec/PRAM-visualize.git
